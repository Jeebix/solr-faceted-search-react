...
class SolrClient {
	...
	setInitialQuery(queryToMerge) {
		...
		this.state.query.searchFields =
		this.state.query.searchFields
			.map((sf) => searchFieldsToMerge // For every searchFields
				.map((sfm) => sfm.field.toString()) // get field(s) as a string
					.indexOf(sf.field.toString()) > -1 // if field in searchFieldsToMerge is found in current search fields
						//
						? {...sf, value: searchFieldsToMerge.find((sfm) => sfm.field.toString() === sf.field.toString()).value
							, invert: searchFieldsToMerge.find((sfm) => sfm.field.toString() === sf.field.toString()).invert}
						//
						: sf
			);

		this.state.query.sortFields = this.state.query.sortFields
			.map((sf) => sortFieldsToMerge.map((sfm) => sfm.field.toString()).indexOf(sf.field.toString()) > -1
				? {...sf, value: sortFieldsToMerge.find((sfm) => sfm.field.toString() === sf.field.toString()).value}
				: sf);
	}
	...
	setSearchMatrixFieldValue(field, value) {
	...
	setInvert(field, value) {
		const { query } = this.state;
		const { searchFields } = query;
		const newFields = searchFields
			.map((searchField) => searchField.field === field ? {...searchField, invert: value} : searchField);
		const payload = {type: "SET_SEARCH_FIELDS", newFields: newFields};
		this.state.query = queryReducer(this.state.query, payload);
		this.onChange(this.state, this.getHandlers());
	}

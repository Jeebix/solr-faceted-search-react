...
class ListFacetItem extends React.Component {
	...
	render() {
		...
		return (
			<div>
				<li className={`facet-item-type-${this.props.facetField}  ${invertedClassName} facet-item-list`}
					key={`${facetVal}_${count}`}
					onClick={onSelect}
					onMouseOver={(e) => this.handleFacetItemOver(e)}
					onMouseOut={(e) => this.handleFacetItemOut(e)}>
					<div className={'list-facet-item-row facet-item-value col-md-9 col-sm-9'}>
						<p className={`managed-wrap ${invertedClassName}`}>{facetVal}</p>
					</div>
					<div className={'facet-item-count col-md-2 col-sm-2'}>
						<p id="facet-value-selection" className={invertedClassName}>
							{/* <small>
					...
					<div className={'facet-item-select col-md-1 col-sm-1'}>
						{value.indexOf(facetVal) > -1 ? <CheckedIcon /> : <UncheckedIcon />}
					</div>
				</li>
			</div>
		);
	...
	render() {
		...
		const showMoreLink = truncateFacetListsAt > -1 && truncateFacetListsAt < facetValues.length ?
			<li className={cx({ "list-group-item": bootstrapCss })} onClick={() => this.setState({ truncateFacetListsAt: -1 })}>
				Tout montrer ({facetValues.length})
			</li> : null;

		const toggleInvertLink = <a id="invert-link" className={cx({ "pull-right": bootstrapCss }, { "inverted": invert })} onClick={this.toggleInvert.bind(this)}>
			{invert ? 'inversé' : 'inverser'}
		</a>;

		const idlistFacetItem = `list-facet-item-${v4()}`;
						...
						<ul className={cx({ "list-group": bootstrapCss })}>
							{
								...
								facetValues.filter((facetValue, i) =>
									truncateFacetListsAt < 0 || i < truncateFacetList)
										.map((facetValue, i) =>
											this.state.filter.length === 0
											|| facetValue.toLowerCase().indexOf(this.state.filter.toLowerCase()) > -1 ?
											(
												<ListFacetItem facetVal={facetValue}
																value={value}
																count={facetCounts[i]}
																facetField={field}
																inverted={invert}
																onSelect={(e) => this.handleClick(e, facetValue)}
																key={`${facetValue}-${idlistFacetItem}`} />
											) : null
										)
							}
							{showMoreLink}
						</ul>
						{ facetValues.length > 6 ? (
							<div>
								<div id="filter-div" className={cx({ "col-md-6 col-sm-6": bootstrapCss, "form-group": bootstrapCss })}>
									<input id="filter-input" className={cx({ "form-control": bootstrapCss, "input-sm": bootstrapCss })}
											onChange={(ev) => this.setState({ filter: ev.target.value })}
											placeholder="Filtrer..." type="text"
											value={this.state.filter} />
								</div>
								<div id="facet-sort-div" className={cx({ "btn-group": bootstrapCss })}>
								...
								<button className={cx({ "col-md-2 col-sm-2": bootstrapCss, "pull-right": bootstrapCss, "btn": bootstrapCss, "btn-default": bootstrapCss, "btn-xs": bootstrapCss })}

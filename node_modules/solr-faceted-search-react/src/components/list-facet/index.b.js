...
class ListFacet extends React.Component {
    ...
    render() {
        ...
        return (
            <li className={cx("list-facet", { "list-group-item": bootstrapCss })}
                id={`solr-list-facet-${field}`}>
                <header onClick={this.toggleExpand.bind(this)}>
                    <h5>
                        {bootstrapCss ? (<span>
                            <span className={cx("glyphicon", { "glyphicon-collapse-down": expanded, "glyphicon-collapse-up": !expanded })} />
                            {" "}
                        </span>)
                            : null}
                        {label}
                ...
                {expanded ? (<div>
                            ...
                            .map((facetValue, i) =>
                                this.state.filter.length === 0 || facetValue.toLowerCase().indexOf(this.state.filter.toLowerCase()) > -1 ?
                                    (<li className={cx(`facet-item-type-${field}`, { "list-group-item": bootstrapCss })}
                                        key={`${facetValue}_${facetCounts[i]}`}
                                        onClick={() => this.handleClick(facetValue)}>
                                        {value.indexOf(facetValue) > -1 ? <CheckedIcon onClick={this.handleCheckBoxOff} />
                                            : <UncheckedIcon onClick={this.handleCheckBoxOn} />}
                                        { /*value.indexOf(facetValue) > -1 ? <CheckedIcon /> : <UncheckedIcon />*/}
                                        {facetValue}
                    ...
                    {facetValues.length > 4 ? (<div>
                        <input onChange={(ev) => this.setState({

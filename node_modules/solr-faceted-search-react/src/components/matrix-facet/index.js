...
class MatrixRow extends React.Component {
	...
	render () {
		...
		const idMatrixCellCDE = `matrix-cell-CDE-${v4()}`;
		const idMatrixCellKIT = `matrix-cell-KIT-${v4()}`;
...
class MatrixFacet extends React.Component {
	...
	toggleCellStatus(current_cell_status) {
		let new_cell_status = current_cell_status;
	...
	mergeMatrix(m) {
		...
		values.map((v) => { 
			...
			(row.length > 0 && col.length) > 0 ? m[row][col]
				.filter((c) => {
					return c.camp == cell;
				})
				.map((c) => c.status = parseInt(state)) : 0
		});
	...
	render() {
		const { query, label, facets, field, value, bootstrapCss, inverted, facetSort, collapse } = this.props;
		...
		const invertedClassName = (inverted && value.indexOf(facetVal) > -1) ? "rejected" : "";

		// const columnHeaderStyle = {textAlign: "center"};
		const idMatrixRow = `matrix-row-${v4()}`;

		return (
			<li className={cx("facet-box","list-facet", {"list-group-item col-md-7 col-sm-12": bootstrapCss})} id={`solr-list-facet-${field}`}>
					...
					<table>
						<thead>
							<tr className={cx('matrix-facet-table')}>
								<th></th>
								<th>
									<button className={cx('matrix-column', {"btn btn-info btn-sm btn-block": bootstrapCss})}>Commandes</button>
								</th>
								<th>
									<button className={cx('matrix-column', {"btn btn-info btn-sm btn-block": bootstrapCss})}>Kits</button>
								</th>
							</tr>
						</thead>
						<tbody>
							{Object.keys(matrix)
									.map((k) => {
										return <MatrixRow row_name={matrix[k].label}
